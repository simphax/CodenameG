...
/**
 * Class that represents a camera with width, height and a position depending on
...
public class Camera {
    ...
    /**
     * Updates the camera's position to match the two players' position or just
     * one player of the other is dead
     * 
     * @param The
     *            time since the last update
     */
    public void update(int elapsedTime) {
	if (world.getAmountOfPlayers() > 0) {
	    for (PlayerCharacter c : world.getPlayers()) {
		players.add(c);
	    }

	    Xmax = players.get(0).getPosition().getX()
		    + players.get(0).getHitbox().getWidth();
	    Xmin = players.get(0).getPosition().getX();
	    Ymax = players.get(0).getPosition().getY()
		    + players.get(0).getHitbox().getHeight();
		...
		Xmax = Math.max(Xmax, players.get(i).getPosition().getX()
			+ players.get(i).getHitbox().getWidth());
	    ...
	    width = Math.min(
		    Math.max((int) (Ymax - Ymin + minHeight), (int) (Xmax
			    - Xmin + minWidth)), maxWidth);
	    height = Math.min(
		    Math.max((int) (Ymax - Ymin + minHeight), (int) (Xmax
			    - Xmin + minWidth)), maxHeight);
